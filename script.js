// ข้อมูลดิบจากไฟล์ของผู้ใช้
const rawDataContent = `หมวดผู้คน
Aunt (n.) ป้า , น้า , อา
Is that lady your aunt?
ผู้หญิงท่านนั้นคือคุณป้า ของคุณใช่ไหมครับ

Cousin (n.) ลูกพี่ลูกน้อ ง
My cousin recommended this place.
ลูกพี่ลูกน้อ งของผมแนะนำที่นี่มาครับ

Daughter (n.) ลูกสาว
Your daughter is very polite.
ลูกสาวของคุณมารยาทเรียบร้อยมากครับ

Husband (n.) สามี
I am her husband.
ผมเป็นสามีของเธอครับ

Uncle (n.) ลุง, น้า , อา
He is my uncle.
ท่านนี้คือคุณลุงของผมเองครับ

Teenager (n.) วัยรุ่น
Is this area popular with teenagers?
แถวนี้เป็นที่นิยมของวัยรุ่นเหรอครับ?



หมวดบทบาท
Guest (n.) แขก, ผู้มาพัก
Is this lounge for guests only?
ห้องรับรองนี้สำหรับแขกเท่านั้นใช่ไหมครับ

Neighbour (n.) เพื่อนบ้าน
I don't want to disturb the neighbours.
ผมไม่อยากรบกวนเพื่อนบ้านครับ

Partner (n.) หุ้นส่วน, คู่รัก, คู่หู
I am waiting for my partner.
ผมกำลังรอแฟน (หรือคู่ของผม) อยู่ครับ

Tourist (n.) นักท่องเที่ยว
Can you recommend a place for tourists?
คุณช่วยแนะนำสถานที่สำหรับนักท่องเที่ยวหน่อยได้ไหม
ครับ

Visitor (n.) ผู้มาเยือน, ผู้มาติดต่อ
Where is the visitor parking?
ที่จอดรถสำหรับผู้มาติดต่ออยู่ตรงไหนครับ

Member (n.) สมาชิก
Can I apply for a member card?
ผมขอสมัครบัตรสมาชิกได้ไหมครับ



หมวดสรรพนาม ความเป็นเจ้าของ

Our (det.) ของเรา, ของพวกเรา
Is this our bus?
นี่ใช่รถบัสของพวกเราไหมครับ

Their (det.) ของพวกเขา
I think that is their table.
ผมคิดว่านั่นเป็นโต๊ะของพวกเขานะครับ

Yourself (pron.) ตัวคุณเอง
Did you make this yourself?
คุณทำสิ่งนี้ด้วยตัวคุณเองเลยเหรอครับ

Everyone (pron.) ทุกคน
Does everyone need a ticket?
ทุกคนต้องใช้ตั๋วไหมครับ

Everybody (pron.) ทุกคน (เน้น ภาษาพูด)
Is everybody here?
ทุกคนมากันครบหรือยังครับ

Someone (pron.) บางคน, ใครบางคน
Is someone sitting here?
มีใครนั่งตรงนี้ไหมครับ



หมวดสรรพนาม ความเป็นเจ้าของ

Somebody (pron.) บางคน, ใครบางคน
Somebody left this bag here.
มีใครบางคนลืมกระเป๋า ใบนี้ไว้ตรงนี้ครับ

Anyone (pron.) ใคร, ใครก็ได้
Is anyone sitting here?
มีใครนั่งตรงนี้ไหมครับ (ถามก่อนนั่งเก้าอี้ว่าง)

Anybody (pron.) ใคร, ใครก็ได้
Does anybody have a pen?
มีใครพอจะมีปากกาบ้างไหมครับ

No one (pron.) ไม่มีใคร
There is no one at the reception. ไม่มีใครอยู่ที่
เคาน์เ ตอร์ต้อนรับเลยครับ (แจ้งเพื่อเรียกพนักงาน)

Nobody (pron.) ไม่มีใคร
Nobody came to pick me up.
ไม่มีใครมารับผมเลยครับ
(แจ้งปัญหาทัวร์หรือรถรับส่งที่นัดไว้)



หมวดอาชีพ
Boss (n.) เจ้านาย, หัวหน้า
Are you the boss?
คุณเป็นเจ้านาย (เจ้าของร้าน)ใช่ไหมครับ?

Manager (n.) ผู้จัดการ
Could I speak to the manager, please?
ผมขอเรียนสาย (หรือขอพบ) ผู้จัดการหน่อยได้ไหมครับ?

Model (n.) นางแบบ, นายแบบ
Is she a famous model?
เธอคือนางแบบที่มีชื่อเสียงใช่ไหมครับ?

Police (n.) ตำรวจ
Where is the nearest police station?
สถานีตำรวจที่ใกล้ที่สุดอยู่ที่ไหนครับ?
(ถามทางเมื่อเกิดเหตุฉุกเฉิน)

Worker (n.) คนงาน, พนักงาน
Can I ask that worker for directions?
ผมขอถามทางพี่คนงานคนนั้นได้ไหมครับ



หมวดอาชีพ
Waiter (n.) พนักงานเสิร์ฟ (ชาย)
Excuse me, do you work as a waiter here?
ขอโทษนะครับ คุณทำงานเป็นพนักงานเสิร์ฟที่นี่หรือเปล่า
ครับ?

หรือ

Is the service charge for the waiter included?
ค่าบริการสำหรับพนักงานเสิร์ฟรวมอยู่ในนี้หรือยังครับ?
(ถามตอนเช็คบิลเพื่อจะได้รู้ว่าต้องให้ทิปเพิ่มไหม)



หมวดการทำงาน
Career (n.) อาชีพ, หน้า ที่การงาน
What is your career back home?
คุณประกอบอาชีพอะไรที่ประเทศของคุณเหรอครับ? (ใช้
ชวนคุยแลกเปลี่ยนเรื่องงานกับเพื่อนต่างชาติ)

Factory (n.) โรงงาน
Is the factory open for tours?
โรงงานนี้เปิดให้เข้าชมไหมครับ?

Job (n.) งาน, อาชีพ
I took a break from my job to travel.
ผมลางานมาเที่ยวครับ / ผมพักจากงานมาเที่ยวครับ

Meeting (n.) การประชุม
Where is the meeting point for the tour?
จุดนัดพบสำหรับทัวร์อยู่ตรงไหนครับ?

Staff (n.) พนักงาน, เจ้าหน้า ที่ (โดยรวม)
Excuse me, where can I find the hotel staff?
ขอโทษครับ ผมจะหาเจ้าหน้า ที่โรงแรมได้ที่ไหนครับ?

Work (v., n.) ทำงาน (กริยา), งาน (นาม)
How does this ticket machine work?
เครื่องขายตั๋วนี้ใช้งานยังไงครับ?



หมวดร่างกาย
Arm (n.) แขน
My arm hurts from carrying the bag.
ผมเจ็บแขนจากการแบกกระเป๋า ครับ

Back (n., adv.) หลัง, ข้างหลัง
Is the entrance at the back?
ทางเข้าอยู่ด้านหลังใช่ไหมครับ? (ใช้ถามทาง)

Body (n.) ร่างกาย, ตัว
I would like a full body massage.
ผมขอคอร์สนวดทั้งตัวนะครับ (ใช้สั่งบริการสปา/นวด)

Ear (n.) หู
My ears are popping.
หูของผมอื้อครับ
(ใช้บอกอาการเวลาขึ้นเครื่องบินหรือขึ้นที่สูง)

Eye (n.) ตา
Do you have eye drops?
คุณมียาหยอดตาขายไหมครับ? (ใช้ถามร้านขายยา)

Face (n.) ใบหน้า , หน้า
There is something on your face.
มีอะไรติดที่หน้า ของคุณนะครับ



หมวดร่างกาย
Foot (n.) เท้า
My feet are sore from walking.
ผมเจ็บเท้าจากการเดิน(มาทั้งวัน)ครับ

Hair (n.) ผม
Excuse me, I found a hair in my food.
ขอโทษนะครับ ผมเจอเส้นผมในอาหารครับ

Where can I get a haircut?
ผมจะไปตัดผมได้ที่ไหนบ้างครับ?

Hand (n.) มือ
Could you give me a hand with this bag?
คุณช่วยผมยกกระเป๋า ใบนี้หน่อยได้ไหมครับ?

Head (n.) ศีรษะ, หัว
Do you have medicine for a headache?
คุณมียาแก้ปวดหัวไหมครับ? (ใช้ถามร้านขายยา)

Leg (n.) ขา
I prefer the chicken leg, please.
ผมขอเป็นส่วนน่องไก่นะครับ (ใช้สั่งไก่ทอด หรือข้าวมันไก่)



หมวดร่างกาย
Mouth (n.) ปาก
The food is so spicy, my mouth is burning.
อาหารเผ็ดมาก ปากผมจะไหม้(เผ็ดร้อน)อยู่แล้วครับ

Nose (n.) จมูก
Do you have medicine for a runny nose?
คุณมียาลดน้ำมูกไหมครับ?

Tooth (n.) ฟัน
Is there a dentist nearby? I have a toothache.
แถวนี้มีหมอฟันไหมครับ? ผมปวดฟัน



หมวดสุขภาพ
Health (n.) สุขภาพ
Please take care of your health.
ดูแลสุขภาพด้วยนะครับ

Healthy (adj.) สุขภาพดี, แข็งแรง
I hope you stay healthy.
ขอให้คุณสุขภาพแข็งแรงนะครับ

Hospital (n.) โรงพยาบาล
Where is the nearest hospital?
โรงพยาบาลที่ใกล้ที่สุดอยู่ที่ไหนครับ?

Medicine (n.) ยา
Where can I buy some medicine?
ผมจะหาซื้อยาได้ที่ไหนบ้างครับ?

Sick (adj.) ป่ว ย, ไม่สบาย
I feel sick, can we stop for a moment?
ผมรู้สึกไม่สบาย เราช่วยแวะพักสักครู่ได้ไหมครับ?

Tired (adj.) เหนื่อย
I am very tired, I want to go back to the hotel.
ผมเหนื่อยมากครับ อยากกลับโรงแรมแล้ว



หมวดสุขภาพ
Die (v.) ตาย, ดับ (แบตหมด)
My phone is about to die.
มือถือของผมแบตกำลังจะหมดครับ (ใช้บอกเพื่อนหรือคน
ขับรถเพื่อรีบหาที่ชาร์จ)

Dead (adj.) ตายแล้ว, ดับ, เสีย
My phone is dead, can I use your charger?
มือถือผมแบตหมดแล้ว ผมขอยืมสายชาร์จคุณหน่อยได้ไหม
ครับ?

Life (n.) ชีวิต
How is the nightlife here?
สีสันยามค่ำคืน (Nightlife) ของที่นี่เป็นอย่างไรบ้างครับ?

Live (v.) อาศัยอยู่
I live in Bangkok. ผมอาศัยอยู่ที่กรุงเทพฯ ครับ

Born (v.) เกิด
I was born in Thailand.
ผมเกิดที่ประเทศไทยครับ



หมวดเครื่องแต่งกาย
Sweater (n.) เสื้อกันหนาว (ไหมพรม)
Do I need to bring a sweater?
ผมจำเป็นตองพกเสื้อกันหนาวไปด้วยไหมครับ?

Trousers (n.) กางเกงขายาว
(British English ใช้ Trousers / American English ใช้
Pants)
Can I try these trousers on?
ผมขอลองกางเกงตัวนี้หน่อยได้ไหมครับ?

Pocket (n.) กระเป๋า เสื้อ, กระเป๋า กางเกง
Let me check my pocket.
เดี๋ยวผมขอเช็กในกระเป๋า กางเกง(เสื้อ)ก่อนนะครับ

Purse (n.) กระเป๋า ใส่เหรียญ, กระเป๋า ถือผู้หญิง
I am looking for a purse for my wife.
ผมกำลังมองหากระเป๋า ถือให้ภรรยาครับ

Wear (v.) สวมใส่
Can I wear shorts inside the temple?
ผมสามารถใส่กางเกงขาสั้นเข้าไปในวัดได้ไหมครับ?
(ประโยคนี้สำคัญมาก ใช้ถามกฎระเบียบการแต่งกายก่อน
เข้าสถานที่สำคัญ)



หมวดที่อยู่อาศัย
Flat (n.) ห้องชุด, แฟลต
(คำนี้มักใช้ในอังกฤษ ความหมายเหมือน apartment)
I am staying in a flat near the station.
ผมพักอยู่ที่แฟลตใกล้ๆ สถานีครับ

Floor (n.) พื้น, ชั้น (ของอาคาร)
Which floor is my room on?
ห้องพักของผมอยู่ชั้นไหนครับ

Garden (n.) สวน
Can I take a walk in the garden?
ผมขอเข้าไปเดินเล่นในสวนได้ไหมครับ

Home (n., adv.) บ้าน, กลับบ้าน
I am going back home tomorrow.
ผมจะเดินทางกลับบ้าน(กลับประเทศ)พรุ่งนี้ครับ

House (n.) บ้าน (เน้น ตัวสิ่งปลูกสร้าง)
You have a beautiful house.
บ้านของคุณสวยมากครับ

Stairs (n.) บันได
Where are the stairs?
บันไดไปทางไหนครับ



หมวดเฟอร์นิเจอร์ สิ่งของ
Chair (n.) เก้าอี้
Excuse me, is this chair taken?
ขอโทษครับ เก้าอี้ตัวนีมีคนนั่งไหมครับ?
(ใช้ถามก่อนจะหยิบเก้าอี้จากโต๊ะอื่นมาใช้)

Cupboard (n.) ตู้เก็บของ (เช่น ตู้กับข้าว, ตู้ถ้วยชาม)
Are there any plates in the cupboard?
มีจานอยู่ในตู้เก็บของบ้างไหมครับ?

Desk (n.) โต๊ะทำงาน
Is there a desk in the room?
ในห้องพักมีโต๊ะทำงานไหมครับ?
(ใช้ถามโรงแรมตอนจองห้อง เผื่อต้องนั่งเคลียร์งาน)

Furniture (n.) เฟอร์นิเจอร์, เครื่องเรือน
I really like the furniture here.
ผมชอบเฟอร์นิเจอร์ของที่นี่มากครับ

Lamp (n.) โคมไฟ
The lamp is not working.
โคมไฟใช้งานไม่ได้ครับ (ใช้แจ้งพนักงานโรงแรมให้มาซ่อม)



หมวดเฟอร์นิเจอร์ สิ่งของ
Light (n., adj.) ไฟ, แสงสว่าง (n.), เบา (adj.)
The light is a bit too bright.
แสงไฟจ้าเกินไปหน่อยนะครับ

Where is the light switch?
สวิตช์ไฟอยู่ตรงไหนครับ
(ใช้ถามเมื่อเข้าห้องพักแล้วหาที่เปิดไฟไม่เจอ)

Painting (n.) ภาพวาด (งานศิลปะ)
That is a beautiful painting.
นั่นเป็นภาพวาดที่สวยงามมากครับ

Shower (n.) ฝักบัว, การอาบน้ำ ฝักบัว
The shower has no hot water.
ฝกบัวไม่มีน้ำ อุ่นครับ (ใช้แจ้งปัญหาโรงแรม)



หมวดสิ่งของในชีวิตประจำวัน

Bottle (n.) ขวด
Can I have a bottle of water, please?
ขอน้ำ เปล่าขวดหนึ่งครับ (ใช้สั่งน้ำ ดื่ม)

Cup (n.) ถ้วย (มีหูจับ เช่น ถ้วยกาแฟ)
I would like a cup of hot coffee.
ผมขอกาแฟร้อนหนึ่งถ้วยครับ (ใช้สั่งเครื่องดื่ม)

Dictionary (n.) พจนานุก รม
Let me check my dictionary for a moment.
เดี๋ยวขอผมเช็กในพจนานุก รมแป๊บ นึงนะครับ

Key (n.) กุญแจ
I think I left my key in the room.
ผมคิดวาผมลืมกุญแจไว้ในห้องครับ (แจ้งพนักงานต้อนรับ)

Money (n.) เงิน
Where can I exchange money?
ผมจะแลกเงินได้ที่ไหนบ้างครับ (ใช้ถามหาร้านแลกเงิน)

Plate (n.) จาน
Could I have an extra plate, please?
ขอจานแบ่งเพิ่มอีกใบได้ไหมครับ
(ใช้เวลาทานอาหารแล้วต้องการจานแบ่งเพิ่ม)



หมวดอาหาร
Onion (n.) หัวหอม
No onions for me, please.
ผมไม่รับหัวหอมนะครับ (ไม่ใส่หัวหอมนะครับ)

Bread (n.) ขนมปัง
Could we have some more bread, please?
พวกเราขอขนมปังเพิ่มหน่อยได้ไหมครับ

Meat (n.) เนื้อสัตว์
Does this dish contain meat?
อาหารจานนี้มีเนื้อสัตว์ไหมครับ

Soup (n.) ซุป, แกงจืด
What is the soup of the day?
ซุปพิเศษประจำวันนี้คือซุปอะไรครับ

Pepper (n.) พริกไทย
Could I have some salt and pepper, please?
ผมขอเกลือกับพริกไทยหน่อยได้ไหมครับ



หมวดอาหาร
Breakfast (n.) อาหารเช้า
What time is breakfast served?
อาหารเช้าเริ่มเสิร์ฟกี่โมงครับ (ถามพนักงานโรงแรม)

Lunch (n.) อาหารกลางวัน
Can you recommend a good place for lunch?
คุณช่วยแนะนำร้านสำหรับมื้อกลางวันอร่อยๆ หน่อยได้ไหม
ครับ (ถามคนท้องถิ่นหรือ Concierge)

Dinner (n.) อาหารเย็น
I would like to make a reservation for dinner.
ผมต้องการจองโต๊ะสำหรับอาหารเย็นครับ
(โทรจองร้านอาหาร)

Meal (n.) มื้ออาหาร
Is the meal included in the tour price?
ราคาทัวร์นี้รวมค่าอาหารมื้อหลักแล้วหรือยังครับ
(ถามบริษัททัวร์)

Picnic (n.) การไปเที่ยวและนำอาหารไปทานนอกบ้าน 
Are we allowed to have a picnic in this park?
สวนนี้อนุญ าตให้ปิกนิก (นำอาหารมาทาน) ได้ไหมครับ



หมวดเกี่ยวกับอาหาร
Thirsty (adj.) กระหายน้ำ , หิวน้ำ
I am thirsty. Is there a vending machine nearby?
ผมหิวน้ำ จังเลย แถวนี้มีตู้กดน้ำ ไหมครับ?

Cook (v.) ทำอาหาร
Did you cook this yourself?
คุณทำอาหารจานนี้เองเลยหรือเปล่าครับ?

I can cook simple meals.
ผมพอจะทำอาหารง่ายๆ เป็นครับ

Cooking (n.) การทำอาหาร
Thank you for your cooking.
ขอบคุณสำหรับอาหารฝีมือคุณนะครับ



หมวดสถานที่
Centre (n.) ศูนย์กลาง, ใจกลาง (เมือง)
How can I get to the city centre?
ผมจะเดินทางไปใจกลางเมืองได้อย่างไรบ้างครับ

City (n.) เมือง (ขนาดใหญ่)
Do you have a city map?
คุณมีแผนที่เมืองแจกไหมครับ

Club (n.) สโมสร, ผับ, ชมรม
Is there a dress code for the club?
คลับนี้มีกฎเรื่องการแต่งกายไหมครับ

Theatre (n.) โรงละคร, โรงหนัง
(British English ใช้ Theatre / American English ใช้
Theater)
What is showing at the theatre tonight?
คืนนี้มีเรื่องอะไรฉาย (หรือแสดง) ที่โรงละครบ้างครับ

Town (n.) เมือง (ขนาดกลาง/เล็ก)
I am new in town.
ผมเพิ่งมาอยู่ที่เมืองนี้ครับ



หมวดการขนส่ง
Flight (n.) เที่ยวบิน
Have a safe flight.
ขอให้เดินทางโดยสวัสดิภาพนะครับ

Plane (n.) เครื่องบิน
What time do we board the plane?
พวกเราจะขึ้นเครื่องบินได้กี่โมงครับ?
(ใช้ถามเจ้าหน้า ที่หน้า  Gate)

Traffic (n.) การจราจร
The traffic is heavy today.
วันนี้รถติดมากครับ

Sorry I am late, the traffic is terrible.
ขอโทษที่มาสายครับ การจราจรแย่มาก (รถติดมาก) ครับ



หมวดการเดินทาง / ทิศทาง
Arrive (v.) มาถึง
What time does the train arrive in Paris?
รถไฟจะไปถึงปารีสกี่โมงครับ?

Cross (v.) ข้าม
Where is the safest place to cross the street?
ตรงไหนข้ามถนนได้ปลอดภัยที่สุดครับ?
(ใช้ถามคนแถวนั้น เพราะบางเมืองการข้ามถนนอาจ
อันตราย)

Fly (v.) บิน, เดินทางโดยเครื่องบิน
We fly back home on Sunday.
พวกเราจะบินกลับบ้านวันอาทิตย์ครับ

Leave (v.) ออกจาก, ทิ้งไว้ (ฝากไว้)
Can I leave my luggage here after check-out?
ผมขอฝากกระเป๋า ไว้ที่นี่หลังจากเช็คเอาท์ได้ไหมครับ?
(ประโยคนี้มีประโยชน์ม าก ใช้ถามโรงแรมเพื่อฝากของ
แล้วไปเที่ยวต่อ)

Road (n.) ถนน
Does this road go to the beach?
ถนนเส้นนี้ไปถึงชายหาดไหมครับ?



หมวดทิศทาง / ตำแหน่ง
East (n., adj., adv.) ทิศตะวันออก
Which way is the East Exit?
ทางออกฝั่ งทิศตะวันออกไปทางไหนครับ?
(ใช้ถามในสถานีรถไฟใหญ่ๆ เพื่อไม่ให้หลงทางออก)

North (n., adj., adv.) ทิศเหนือ
The map shows North at the top.
ในแผนที่ด้านบนคือทิศเหนือครับ

South (n., adj., adv.) ทิศใต้
We are traveling to the South of Thailand.
พวกเรากำลังจะเดินทางไปเที่ยวภาคใต้ของไทยครับ

West (n., adj., adv.) ทิศตะวันตก
My room faces West, so it is hot in the afternoon.
ห้องของผมหันหน้า ไปทางทิศตะวันตก ตอนบ่ายเลยร้อน
ครับ (ใช้ขอเปลี่ยนห้องพัก)

Left (adj., adv., n.) ซ้าย
The toilet is on your left-hand side.
ห้องน้ำ จะอยู่ทางซ้ายมือของคุณครับ



หมวดทิศทาง / ตำแหน่ง
Right (adj., adv., n.) ขวา, ถูกต้อง
Is this the right way to the museum?
ทางนี้ไปพิพิธภัณฑ์ถูกทางไหมครับ?

Front (n., adj.) ด้านหน้า
Please drop me off in front of the hotel.
ช่วยจอดส่งผมที่หน้า โรงแรมหน่อยครับ

Side (n.) ด้านข้าง
I would like the sauce on the side, please.
ผมขอแยกซอสมาไว้ด้านข้าง (ยังไม่ต้องราดลงไป) นะครับ 

Top (n.) ด้านบน, ยอด
The swimming pool is on the top floor.
สระว่ายน้ำ อยู่ชั้นบนสุดครับ



หมวดโรงเรียน
Board (n.) กระดาน
(ในบริบทท่องเที่ยวหมายถึง ป้า ยบอกเวลาเที่ยวบิน/รถไฟ)
Please check the departure board for your gate
number.
กรุณาตรวจสอบหมายเลขประตูทางออกจากป้า ยแสดงเที่ยว
บินขาออกนะครับ

Class (n.) ชั้นเรียน, คาบเรียน
The class starts at nine o'clock.
ชั้นเรียนเริ่มตอนเก้าโมงเช้านะครับ

Classroom (n.) ห้องเรียน
The classroom is open. ห้องเรียนเปิดอยู่นะครับ

Course (n.) หลักสูตร, รายวิชา
This course is very useful.
หลักสูตรนี้มีประโยชน์ม ากครับ

Exercise (n., v.) แบบฝึกหัด (n.), ออกกำลังกาย (v.)
This exercise is good for you.
แบบฝึกหัดนี้ดีต่อคุณนะครับ

Lesson (n.) บทเรียน
The lesson is quite easy. บทเรียนนี้ค่อนข้างง่ายนะครับ



หมวดภาษา
Dialogue (n.) บทสนทนา
The dialogue in this movie is very fast.
บทสนทนาในหนังเรื่องนี้พูดเร็วมากครับ

Letter (n.) จดหมาย, ตัวอักษร
Please write your name in capital letters.
กรุณาเขียนชื่อของคุณเป็นตัวพิมพ์ใหญ่นะครับ
(ประโยคนี้สำคัญมาก! ใช้บ่อยเวลาเจ้าหน้า ที่บอกให้เรากรอก
เอกสารเข้าเมืองหรือเช็คอินโรงแรม)

Meaning (n.) ความหมาย
What is the meaning of this word?
ความหมายของคำนี้คืออะไรครับ

Note (n.) บันทึก, โน้ต  (ข้อความสั้นๆ)
I left a note for the housekeeper on the bed.
ผมเขียนโน้ต ทิ้งไว้ให้แม่บ้านวางไว้บนเตียงครับ (เช่น เขียน
ขอหมอนเพิ่ม หรือให้ทิป)

Say (v.) พูด, กล่าว
How do you say "Thank you" in Thai?
คุณพูดคำว่า "ขอบคุณ" ในภาษาไทยว่าอย่างไรครับ



หมวดภาษา
Speak (v.) พูด (สนทนา/ความสามารถทางภาษา)
Can you speak Thai?
คุณพูดภาษาไทยได้ไหมครับ

Could you speak a little slower, please?
รบกวนช่วยพูดช้าลงหน่อยได้ไหมครับ

Talk (v.) คุย, สนทนา
Can we talk for a minute?
เราคุยกันสักครู่ได้ไหมครับ

Who should I talk to if I have a problem?
ผมควรจะคุยกับใครถ้าหากเกิดมีปัญหาครับ



หมวดความคิด
Decide (v.) ตัดสินใจ
I haven't decided what to order yet.
ผมยังตัดสินใจไม่ได้เลยว่าจะสั่งอะไรดีครับ (ใช้บอก
พนักงานเสิร์ฟเพื่อให้เขารอก่อน หรือให้เขาแนะนำเมนู)

Guess (v.) เดา
Can you guess the answer?
คุณพอจะเดาคำตอบได้ไหมครับ

I guess we are lost.
ผมเดาว่าพวกเราหลงทางกันแล้วล่ะครับ

Hope (v.) หวัง
I hope to see you again someday.
หวังว่าสักวันคงได้พบคุณอีกนะครับ

Imagine (v.) จินตนาการ, นึกคิด
I can't imagine how expensive it is.
ผมจินตนาการไม่ออกเลยว่ามันจะแพงขนาดไหน

Learn (v.) เรียนรู้
It is never too late to learn.
ไม่มีคำว่าสายเกินไปสำหรับการเรียนรู้นะครับ



หมวดธรรมชาติ
Grass (n.) หญ้า
Is it okay to sit on the grass?
นั่งบนสนามหญ้าได้ไหมครับ?

Island (n.) เกาะ
How long does the boat take to get to the island?
นั่งเรือไปที่เกาะใช้เวลานานเท่าไหร่ครับ?

Land (n.) แผ่นดิน, ทางบก
We prefer to travel by land.
พวกเราขอเลือกเดินทางทางบกดีกว่าครับ (ใช้บอกเอเจนซี่
ทัวร์ กรณีไม่อยากนั่งเครื่องบินหรือเรือ อยากนั่งรถไฟหรือรถ
บัสแทน)

Plant (n.) พืช, ต้นไม้
What kind of plant is this?
ต้นไม้นี้คือต้นอะไรครับ?

River (n.) แม่น้ำ
I would like a room with a river view.
ผมขอห้องพักวิวแม่น้ำ ครับ



หมวดธรรมชาติ
Sea (n.) ทะเล
How far is the hotel from the sea?
โรงแรมอยู่ไกลจากทะเลแค่ไหนครับ?
(ใช้ถามตำแหน่งที่ตั้งเพื่อดูว่าเดินไปหาดได้ไหม)

Sky (n.) ท้องฟ้า
The sky is getting dark; it might rain soon.
ท้องฟ้า เริ่มมืดแล้ว ฝนน่าจะตกเร็วๆ นี้นะครับ
(ใช้เตือนเพื่อนร่วมทริปให้เตรียมร่มหรือหาที่หลบฝน)

Space (n.) อวกาศ, พื้นที่ว่าง
Do you have enough space in your luggage?
ในกระเป๋า เดินทางของคุณมีที่ว่างพอไหมครับ? (ใช้ถาม
เพื่อนเวลาซื้อของฝากเยอะๆ แล้วกลัวใส่กระเป๋า ไม่หมด)



หมวดสภาพอากาศ
Cold (adj., n.) หนาว, เย็น
It is very cold today. 
วันนี้อากาศหนาวมากครับ

Warm (adj.) อบอุ่น
It is warm inside.
ข้างในอากาศอบอุ่นนะครับ

Weather (n.) สภาพอากาศ
The weather is nice today.
วันนี้อากาศดีนะครับ



หมวดวัน
Monday (n.) วันจันทร์
Most museums are closed on Mondays.
พิพิธภัณฑ์ส่วนใหญ่จะปิดทำการในวันจันทร์นะครับ

Tuesday (n.) วันอังคาร
I will let you know by Tuesday.
ผมจะแจ้งให้คุณทราบภายในวันอังคารนะครับ

Wednesday (n.) วันพุธ
Is the market open on Wednesdays?
ตลาดเปิดขายในวันพุธไหมครับ

Thursday (n.) วันพฤหัสบดี
My flight is next Thursday.
เที่ยวบินของผมคือวันพฤหัสบดีหน้า ครับ

Friday (n.) วันศุกร์
Do you have plans for Friday night?
คืนวันศุกร์คุณมีแผนจะไปไหนหรือยังครับ

Saturday (n.) วันเสาร์
We are fully booked on Saturday.
วันเสาร์เราถูกจองเต็มแล้วครับ

Sunday (n.) วันอาทิตย์
Parking is free on Sundays.
วันอาทิตย์จอดรถฟรีนะครับ



หมวดเดือน
January (n.) เดือนมกราคม
It is very cold in January.
เดือนมกราคมอากาศหนาวมากครับ

February (n.) เดือนกุมภาพันธ์
My birthday is in February.
วันเกิดของผมอยู่ในเดือนกุมภาพันธ์ครับ

March (n.) เดือนมีนาคม
The project starts in March.
โครงการจะเริ่มในเดือนมีนาคมครับ

April (n.) เดือนเมษายน
It is hot in April.
เดือนเมษายนอากาศร้อนนะครับ

May (n.) เดือนพฤษภาคม
I will visit you in May.
ผมจะไปเยี่ยมคุณเดือนพฤษภาคมนะครับ



หมวดเดือน
June (n.) เดือนมิถุนายน
We are getting married in June.
เรากำลังจะแต่งงานกันเดือนมิถุนายนครับ

July (n.) เดือนกรกฎาคม
School ends in July.
โรงเรียนปิดเทอมเดือนกรกฎาคมครับ

August (n.) เดือนสิงหาคม
It rains a lot in August.
ฝนตกเยอะมากในเดือนสิงหาคมครับ

September (n.) เดือนกันยายน
I start my new job in September.
ผมเริ่มงานใหม่เดือนกันยายนครับ

October (n.) เดือนตุลาคม
Halloween is in October.
วันฮาโลวีนอยู่ในเดือนตุลาคมครับ



หมวดเดือน
November (n.) เดือนพฤศจิกายน
The weather changes in November.
อากาศเริ่มเปลี่ยนในเดือนพฤศจิกายนครับ

December (n.) เดือนธันวาคม
See you in December.
แล้วเจอกันเดือนธันวาคมนะครับ



หมวดฤดู
Spring (n.) ฤดูใบไม้ผลิ
Flowers bloom in spring.
ดอกไม้บานในฤดูใบไม้ผลินะครับ

Summer (n.) ฤดูร้อน
I love summer.
ผมชอบฤดูร้อนครับ

Autumn (n.) ฤดูใบไม้ร่วง
Leaves fall in autumn.
ใบไม้ร่วงในฤดูใบไม้ร่วงนะครับ

Winter (n.) ฤดูหนาว
Do you like winter?
คุณชอบฤดูหนาวไหมครับ



หมวดหน่วยเวลา
Day (n.) วัน, กลางวัน
Have a nice day.
ขอให้เป็นวันที่ดีนะครับ

Hour (n.) ชั่วโมง
I will be back in an hour.
ผมจะกลับมาในอีกหนึ่งชั่วโมงครับ

Minute (n.) นาที
Just a minute, please.
รอสักครู่นะครับ (สักนาทีเดียว)

Month (n.) เดือน
See you next month.
เจอกันเดือนหน้า นะครับ

O'clock (adv.) ...นาฬิกา (บอกเวลาตรง)
It is ten o'clock.
ตอนนี้สิบโมงแล้วครับ



หมวดหน่วยเวลา
Date (n.) วันที่, การนัดเดต
What is the date today?
วันนี้วันที่เท่าไหร่ครับ

Second (n.) วินาที (หน่วยเวลา)
Wait a second.
รอสักวินาทีนะครับ (รอเดี๋ยวเดียว)

Time (n.) เวลา
What time is it?
กี่โมงแล้วครับ

Week (n.) สัปดาห์, อาทิตย์
I have a busy week.
สัปดาห์นี้ผมยุ่งมากครับ

Year (n.) ปี
Happy New Year!
สวัสดีปีใหม่ครับ



หมวดช่วงเวลา
Afternoon (n.) ตอนบ่าย
Good afternoon. สวัสดีตอนบ่ายครับ

Birthday (n.) วันเกิด
Happy birthday! สุขสันต์วันเกิดนะครับ

Evening (n.) ตอนเย็น
Good evening. สวัสดีตอนเย็นครับ

Holiday (n.) วันหยุด
I am on holiday. ผมอยู่ในช่วงวันหยุดครับ

Midnight (n.) เที่ยงคืน
The shop closes at midnight. ร้านปิดตอนเที่ยงคืนครับ

Moment (n.) ชั่วครู่, ขณะ
One moment, please. กรุณารอสักครู่นะครับ

Morning (n.) ตอนเช้า
Good morning. สวัสดีตอนเช้าครับ



หมวดช่วงเวลา
Night (n.) กลางคืน
Good night. ราตรีสวัสดิ์ครับ (ฝันดีครับ)

Today (adv., n.) วันนี้
Are you busy today? วันนี้คุณยุ่งไหมครับ

Tomorrow (adv., n.) พรุ่งนี้
See you tomorrow. เจอกันพรุ่งนี้นะครับ

Tonight (adv., n.) คืนนี้
Are you free tonight? คืนนี้คุณว่างไหมครับ

Week (n.) สัปดาห์, อาทิตย์
I have a busy week. สัปดาห์นี้ผมยุ่งมากครับ

Weekend (n.) สุดสัปดาห์ (เสาร์-อาทิตย์)
Have a good weekend.
ขอให้มีความสุขในวันหยุดสุดสัปดาห์นะครับ

Yesterday (adv., n.) เมื่อวาน
I saw him yesterday.
ผมเจอเขาเมื่อวานครับ



หมวดกริยาวิเศษณ์บ อกเวลา
Ago (adv.) ที่ผ่านมา, ล่วงมาแล้ว
I arrived two days ago. ผมมาถึงเมื่อสองวันที่แล้วครับ

Always (adv.) สม่ำเสมอ, ตลอด
I always drink coffee. ผมดื่มกาแฟเป็นประจำครับ

Early (adj., adv.) เช้า, ก่อนเวลา
You are early today. วันนี้คุณมาเช้านะครับ

Ever (adv.) เคย
Have you ever been here? คุณเคยมาที่นี่ไหมครับ

Late (adj., adv.) สาย, ช้า, ดึก
Sorry I am late. ขอโทษที่ผมมาสายครับ

Later (adv.) ภายหลัง, เดี๋ยว...
See you later. ไว้เจอกันใหม่นะครับ

Never (adv.) ไม่เคย
I never eat meat. ผมไม่เคยทานเนื้อสัตว์ครับ



หมวดกริยาวิเศษณ์บ อกเวลา
Then (adv.) ตอนนั้น, หลังจากนั้น
Turn left, and then go straight.
เลี้ยวซ้าย แล้วหลังจากนั้นก็ตรงไปเลยครับ

Okay, then I will take it.
โอเคครับ งั้นผมเอาอันนี้ครับ
(ใช้พูดปิดการขาย หรือตกลงซื้อ)

Usually (adv.) โดยปกติ
I usually wake up at six.
ปกติผมตื่นนอนตอนหกโมงครับ

How long does it usually take?
ปกติใช้เวลานานแค่ไหนครับ
(ใช้ถามระยะเวลาเดินทางหรือรอคิว)



หมวดลำดับ
First (det./number) ลำดับที่หนึ่ง, แรก
This is my first visit.
นี่เป็นการมาเยือนครั้งแรกของผมครับ

Second (det./number) ลำดับที่สอง
Turn right at the second traffic light.
เลี้ยวขวาตรงไฟแดงที่สองนะครับ

Third (det./number) ลำดับที่สาม
Take the third turning on the right.
เลี้ยวขวาที่แยกที่สามนะครับ

Fourth (det./number) ลำดับที่สี่
Today is the fourth of July.
วันนี้วันที่สี่กรกฎาคมครับ

Fifth (det./number) ลำดับที่ห้า
He finished in fifth place.
เขาเข้าเส้นชัยเป็นลำดับที่ห้าครับ



หมวดเงินตรา / การวัด
Bill (n.) บิล, ใบแจ้งหนี้ , ธนบัตร
Can I have the bill, please?
ขอเก็บเงินด้วยครับ (ขอบิลด้วยครับ)

Cent (n.) เซนต์ (หน่วยเงินย่อย)
I don't have any cents.
ผมไม่มีเหรียญย่อย (เซนต์) เลยครับ
(ใช้บอกเวลาพนักงานถามหาเศษเหรียญ)

Pound (n.) ปอนด์ (เงิน หรือ น้ำ หนัก)
How much is it in pounds?
คิดเป็นเงินปอนด์ราคาเท่าไหร่ครับ

Quarter (n.) หนึ่งในสี่, 15 นาที, เหรียญ 25 เซนต์
It is a quarter past ten.
ตอนนี้สิบโมงสิบห้านาทีแล้วครับ

I need a quarter for the cart.
ผมต้องใช้เหรียญควอเตอร์สำหรับรถเข็นครับ (ใช้ในสนาม
บินหรือซูเปอร์มาร์เก็ตที่ต้องหยอดเหรียญมัดจำรถเข็น)



หมวดทั่วไป
Become (v.) กลายเป็น, เริ่ม...
The weather has become quite cold.
อากาศเริ่มหนาวแล้วนะครับ

Begin (v.) เริ่มต้น
What time does the tour begin?
ทัวร์เริ่มกี่โมงครับ (ใช้ถามไกด์หรือเคาน์เ ตอร์ทัวร์)

Break (v., n.) ทำแตก, แลกเงิน(ย่อย)
Can you break a large bill?
คุณพอจะแตกแบงก์ใหญ่ให้หน่อยได้ไหมครับ

Bring (v.) นำมา, พามา
Could you bring me the menu, please?
รบกวนช่วยนำเมนูมาให้ผมหน่อยครับ

Build (v.) สร้าง
When was this castle built?
ปราสาทแห่งนี้สร้างขึ้นเมื่อไหร่ครับ

Call (v., n.) โทรหา, เรียก
Please call a taxi for me.
รบกวนช่วยเรียกแท็กซี่ให้ผมหน่อยครับ
(ใช้บอกพนักงานโรงแรมหรือรปภ.)



หมวดทั่วไป
Carry (v.) ถือ, แบก, หิ้ว
Can I carry this bag onto the plane?
ผมหิ้วกระเป๋า ใบนี้ขึ้นเครื่องได้ไหมครับ

Catch (v.) จับ, ขึ้นรถทัน
I need to catch the bus. ผมต้องรีบไปขึ้นรถบัสครับ

Choose (v.) เลือก
"It is hard to choose; everything looks delicious."
เลือกยากจังเลยครับ น่าทานไปหมดทุกอย่างเลย

Compare (v.) เปรียบเทียบ
Please compare the prices.
ลองเปรียบเทียบราคาดูนะครับ

Complete (adj., v.) สมบูรณ์,  ทำให้เสร็จ
Please complete the form.
กรุณากรอกแบบฟอร์มให้ครบถ้วนนะครับ

Cost (n., v.) ราคา, มีราคา
How much does it cost? ราคาเท่าไหร่ครับ

Cut (v.) ตัด
Please cut the cake. เชิญตัดเค้กได้เลยครับ



หมวดทั่วไป
Describe (v.) บรรยาย, อธิบายลักษณะ
Can you describe him?
คุณช่วยบรรยายลักษณะของเขาหน่อยได้ไหมครับ

Discuss (v.) หารือ, อภิปราย
We need to discuss this. เราต้องหารือเรื่องนีกันนะครับ

Enjoy (v.) เพลิดเพลิน, สนุก กับ
Enjoy your meal. ทานให้อร่อยนะครับ

Explain (v.) อธิบาย
Can you explain it to me?
ช่วยอธิบายให้ผมฟังหน่อยได้ไหมครับ

Fall (v.) ตก, ล้ม
Be careful not to fall.
ระวังล้มนะครับ

Fill (v.) เติม
Please fill the glass.
รบกวนเติมน้ำ ให้เต็มแก้วนะครับ



หมวดทั่วไป
Form (n., v.) แบบฟอร์ม
Do I need to fill out this form?
ผมต้องกรอกแบบฟอร์มนี้ไหมครับ

Get (v.) ได้รับ, เอา, ไปถึง, เข้าใจ
I get it now. ผมเข้าใจแล้วครับ

Grow (v.) เติบโต, ปลูก
Is this grown locally? อันนี้ปลูกแถวนี้หรือเปล่าครับ

Happen (v.) เกิดขึ้น
What happened? เกิดอะไรขึ้นครับ

Hear (v.) ได้ยิน
Can you hear me? คุณได้ยินผมไหมครับ

Improve (v.) ปรับปรุง, พัฒนา
I want to improve my English.
ผมอยากพัฒนาภาษาอังกฤษครับ



หมวดทั่วไป
Include (v.) รวม
Does the price include breakfast?
ราคานี้รวมอาหารเช้าด้วยไหมครับ

Introduce (v.) แนะนำ (ตัว/ให้รู้จัก)
Let me introduce myself.
ขออนุญ าตแนะนำตัวนะครับ

Join (v.) เข้าร่วม
Would you like to join us?
คุณอยากมาร่วมกับเราไหมครับ

Keep (v.) เก็บ, รักษา
Please keep it a secret.
รบกวนเก็บเป็นความลับนะครับ

Laugh (v., n.) หัวเราะ
She is laughing while watching the comedy show.
เธอกำลังหัวเราะขณะดูรายการตลก

Let (v.) อนุญ าต, ขอให้ (เช่น Let's)
Let me check the map.
ขอผมเช็คแผนที่แป๊บ นะครับ



หมวดทั่วไป
Lie (v.) โกหก (หรือ นอนลง)
I need to lie down for a bit.
ผมขอตัวไปนอนพักสักหน่อยนะครับ

Lose (v.) แพ้, ทำหาย
I lost my room key.
ผมทำกุญแจห้องหายครับ

Match (n., v.) การแข่งขัน, เข้ากัน
The colors match well. สีเข้ากันดีนะครับ

Mean (v.) หมายความว่า
What does this mean? อันนี้หมายความว่าอะไรครับ

Miss (v.) คิดถึง, พลาด (รถ/โอกาส)
I miss you. ผมคิดถึงคุณครับ

Move (v.) ย้าย, ขยับ
Can I move to that table?
ผมขอย้ายไปนั่งโต๊ะนั้นได้ไหมครับ

Order (n., v.) สั่ง (อาหาร/ของ)
I would like to order, please.
ขอสั่งอาหารหน่อยครับ



หมวดทั่วไป
Paint (v., n.) ทาสี, วาดภาพ
I will paint the room blue. ผมจะทาสีห้องเป็นสีฟ้า นะครับ

Park (n., v.) สวนสาธารณะ, จอดรถ
Can I park here?
ผมจอดรถตรงนี้ได้ไหมครับ

Plan (n., v.) วางแผน
What is your plan? แผนของคุณคืออะไรครับ

Post (n., v.) ส่งไปรษณีย์
I need to post a letter. ผมต้องไปส่งจดหมายครับ

Practise / Practice (v., n.) ฝึกฝน
I want to practice my English.
ผมอยากฝึกภาษาอังกฤษครับ

Prefer (v.) ชอบมากกว่า
I prefer tea to coffee. ผมชอบชามากกว่ากาแฟครับ

Prepare (v.) เตรียม
I need a few minutes to prepare.
ขอเวลาผมเตรียมตัวแป๊บ นะครับ



หมวดทั่วไป
Put (v.) วาง, ใส่
Where can I put my luggage?
ผมวางกระเป๋า เดินทางไว้ตรงไหนได้บ้างครับ

Repeat (v.) พูดซ้ำ, ทำซ้ำ
Can you repeat that, please?
รบกวนพูดซ้ำอีกทีได้ไหมครับ

Report (n., v.) รายงาน, แจ้งความ
I need to report a lost passport.
ผมต้องการแจ้งความเรื่องพาสปอร์ตหายครับ

See (v.) เห็น, เข้าใจ, พบ
I see what you mean. ผมเข้าใจที่คุณพูดครับ

Shop (n., v.) ร้านค้า, ซื้อของ
Where is a good place to shop around here?
แถวนี้มีที่ไหนน่าเดินซื้อของบ้างครับ

Show (v., n.) แสดง, ให้ดู
Can you show me on the map?
ช่วยชี้ให้ดูในแผนที่หน่อยได้ไหมครับ



หมวดทั่วไป
Spend (v.) ใช้ (เงิน/เวลา)
I don't want to spend too much money.
ผมไม่อยากใช้เงินเยอะเกินไปครับ

Take (v.) พาไป, เอาไป, ใช้เวลา
Can you take me to this address?
ช่วยไปส่งผมตามที่อยู่นี้หน่อยได้ไหมครับ

Tell (v.) บอก, เล่า
Could you tell me the way?
ช่วยบอกทางผมหน่อยได้ไหมครับ

Try (v.) ลอง, พยายาม
Can I try this on? ผมขอลองใส่ตัวนี้ได้ไหมครับ

Turn (v., n.) เลี้ยว, หมุน
Please turn left here. เลี้ยวซ้ายตรงนี้เลยครับ

Worry (v.) กังวล
Don't worry, it's okay. ไม่ต้องกังวลครับ ไม่เป็นไร



หมวดความรู้สึก
Afraid (adj.) กลัว, เกรงว่า
I'm afraid I don't understand.
ผมเกรงว่าผมจะไม่เข้าใจน่ะครับ

Angry (adj.) โกรธ, โมโห
Please don't be angry. อย่าโกรธเลยนะครับ

Bored (adj.) เบื่อ (รู้สึกเบื่อ)
Are you bored with the movie?
คุณเบื่อหนังเรื่องนี้หรือเปล่าครับ

Boring (adj.) น่าเบื่อ
The lecture was not boring.
การบรรยายไม่น่าเบื่อเลยครับ

Excited (adj.) ตื่นเต้น (รู้สึกตื่นเต้น)
I am excited about the news.
ผมตื่นเต้นกับข่าวนี้มากครับ

Exciting (adj.) น่าตื่นเต้น
It is an exciting opportunity.
เป็นโอกาสที่น่าตื่นเต้นมากครับ



หมวดความรู้สึก
Famous (adj.) มีชื่อเสียง
He is a famous actor. เขาเป็นนักแสดงที่มีชื่อเสียงครับ

Interested (adj.) สนใจ (รู้สึกสนใจ)
Are you interested in art? คุณสนใจศิลปะไหมครับ

Interesting (adj.) น่าสนใจ
That is an interesting idea.
นั่นเป็นความคิดที่น่าสนใจครับ

Sorry (adj., exclam.) เสียใจ, ขอโทษ
I am sorry for being late. ขอโทษที่ผมมาสายนะครับ

Terrible (adj.) แย่มาก, เลวร้าย
The traffic today is terrible. วันนี้รถติดแย่มากเลยครับ

Tired (adj.) เหนื่อย
You look a little tired.
คุณดูเหนื่อยๆ นะครับ (แสดงความห่วงใย)



หมวดความรู้สึก
Wonderful (adj.) ยอดเยี่ยม, มหัศจรรย์
Have a wonderful trip.
ขอให้เป็นการเดินทางที่ยอดเยี่ยมนะครับ

Worried (adj.) กังวล
Please don't be worried. ไม่ต้องกังวลนะครับ



หมวดคุณสมบัติ
Best (adj.) ดีที่สุด
What is the best way to get there?
เดินทางไปที่นั่นวิธีไหนดีที่สุดครับ

Better (adj.) ดีกว่า
This one looks better. อันนี้ดูดีกว่านะครับ

Cheap (adj.) ราคาถูก
It is very cheap. ราคาถูกมากเลยครับ

Clear (adj.) ชัดเจน, ใส
Is the explanation clear? คำอธิบายชัดเจนไหมครับ
Everything is clear, thank you.
ทุกอย่างชัดเจนครับ ขอบคุณครับ

Dark (adj.) มืด
It is getting dark. ฟ้า เริ่มมืดแล้วครับ

Dear (adj.) แพง (ภาษาเก่า), เป็นที่รัก
He is a dear friend. เขาเป็นเพื่อนรักของผมครับ

Different (adj.) แตกต่าง
They are very different. พวกมันแตกต่างกันมากครับ



หมวดคุณสมบัติ
Difficult (adj.) ยาก
Is it difficult? มันยากไหมครับ

Extra (adj.) พิเศษ, เพิ่มเติม
Is there an extra charge?
มีค่าใช้จ่ายเพิ่มเติมไหมครับ

Fantastic (adj.) ยอดเยี่ยม, มหัศจรรย์
That sounds fantastic. ฟังดูยอดเยี่ยมมากครับ

Fat (adj.) อ้วน (คน), ไขมัน (อาหาร)
This cat is very fat. แมวตัวนี้อ้วนจังครับ

Fine (adj.) สบายดี, ดี, ค่าปรับ
I am fine, thank you. ผมสบายดีครับ ขอบคุณครับ

Free (adj.) ฟรี, ว่าง, อิสระ
Are you free tomorrow? พรุ่งนี้คุณว่างไหมครับ

Friendly (adj.) เป็นมิตร
The staff are very friendly. พนักงานเป็นมิตรมากครับ



หมวดคุณสมบัติ
Full (adj.) เต็ม, อิ่ม
I am full. ผมอิ่มแล้วครับ

Hard (adj., adv.) ยาก, แข็ง, หนัก
He works very hard. เขาทำงานหนักมากครับ

Heavy (adj.) หนัก
This bag is heavy. กระเป๋า ใบนี้หนักนะครับ

Kind (adj.) ใจดี (ถ้าเป็น n. แปลว่า ประเภท)
You are very kind. คุณใจดีมากครับ

Light (adj.) เบา, สว่าง, อ่อน (สี)
This box is very light. กล่องนี้เบามากครับ

Local (adj.) ท้องถิ่น
I like local food. ผมชอบอาหารท้องถิ่นครับ

Long (adj., adv.) ยาว, นาน
It is a long story. เรื่องมันยาวครับ



หมวดคุณสมบัติ
Negative (adj.) เชิงลบ, ปฏิเสธ
The result is negative. ผลเป็นลบครับ

Nice (adj.) ดี, น่ารัก, นิสัยดี
It is a nice day. วันนี้อากาศดีนะครับ

Only (adj., adv.) เท่านั้น, เดียว
Cash only, please. รับเฉพาะเงินสดนะครับ

Opposite (adj., n., prep., adv.) ตรงข้าม
It is opposite the bank. อยู่ตรงข้ามธนาคารครับ

Poor (adj.) ยากจน, น่าสงสาร, คุณภาพแย่
The signal is poor here. สัญญาณตรงนี้ไม่ค่อยดีเลยครับ

Positive (adj.) เชิงบวก
Please stay positive. มองโลกในแง่บวกไว้นะครับ

Possible (adj.) เป็นไปได้
Is it possible? มันเป็นไปได้ไหมครับ



หมวดคุณสมบัติ
Pretty (adj., adv.) สวย, ค่อนข้าง
It is pretty good. มันค่อนข้างดีเลยครับ

Quick (adj.) เร็ว, ไว
Have a quick look. ลองดูผ่านๆ (ดูเร็วๆ) หน่อยนะครับ

Quiet (adj.) เงียบ
Please be quiet. รบกวนเงียบหน่อยนะครับ

Real (adj.) จริง, แท้
Is this real leather? อันนี้หนังแท้ไหมครับ

Rich (adj.) รวย, เข้มข้น (รสชาติ)
The sauce is very rich. ซอสรสชาติเข้มข้นมากครับ

Right (adj., adv., n.) ถูกต้อง, ขวา, สิทธิ
You are right. คุณพูดถูกครับ

Same (adj., pron., adv.) เหมือนกัน
I feel the same. ผมก็รู้สึกเหมือนกันครับ



หมวดคุณสมบัติ
Similar (adj.) คล้ายกัน
They look similar. ดูคล้ายกันเลยนะครับ

Tall (adj.) สูง
He is very tall. เขาสูงมากเลยครับ

Useful (adj.) มีประโยชน์
This information is useful. ข้อมูลนี้มีประโยชน์ค รับ

Warm (adj.) อบอุ่น
It is warm today. วันนี้อากาศอบอุ่นนะครับ



หมวดคำนำหน้า นามและคำบ่งชี้ต่างๆ
Another (det., pron.) อีกอัน, อื่น
Can I have another glass? ผมขออีกแก้วได้ไหมครับ

Any (det., pron.) บ้าง, ใดๆ, เลย
Do you have any rooms available?
มีห้องว่างบ้างไหมครับ

Both (det., pron.) ทั้งสอง, ทั้งคู่
I will take both. ผมเอาทั้งสองอันเลยครับ

Each (det., pron., adv.) แต่ละ, ละ
How much is each? ราคาอันละเท่าไหร่ครับ

Every (det.) ทุกๆ
I exercise every day. ผมออกกำลังกายทุกวันครับ

Few (det., adj., pron.) น้อ ย, สองสาม...
I have a few minutes.
ผมพอมีเวลาสักสองสามนาทีครับ

Last (det.) สุดท้าย, ล่าสุด
What time is the last train?
รถไฟเที่ยวสุดท้ายกี่โมงครับ



หมวดคำนำหน้า นามและคำบ่งชี้ต่างๆ
Many (det., pron.) มาก (ใช้กับนามนับได้)
There are many people here. ที่นี่คนเยอะมากเลยครับ

More (det., pron., adv.) มากกว่า, เพิ่มอีก
Can I have some more water?
ขอน้ำ เพิ่มหน่อยได้ไหมครับ

Most (det., pron., adv.) ส่วนใหญ่, ที่สุด
Which dish is the most popular?
เมนูไหนนิยมที่สุดครั

Much (det., pron., adv.) มาก (ใช้กับนามนับไม่ได้)
Thank you very much. ขอบคุณมากครับ

Other (adj., pron.) อื่นๆ
Do you have any other ideas?
คุณมีความคิดเห็นอื่นๆ อีกไหมครับ

Same (adj., pron., adv.) เหมือนกัน, เดียวกัน
I will have the same, please.
ผมขอแบบเดียวกันครับ (ใช้เวลาสั่งอาหารตามคนอื่น)



หมวดคำนำหน้า นามและคำบ่งชี้ต่างๆ
Some (det., pron.) บ้าง, บาง
Can I have some tissues?
ผมขอทิชชู่หน่อยได้ไหมครับ

Such (det., pron.) เช่นนั้น, มากขนาดนั้น
It is such a nice day.
วันนี้อากาศดีจริงๆ ครับ

That (det., pron., conj.) นั่น, นั้น
That looks great.
นั่นดูยอดเยี่ยมมากครับ

This (det., pron.) นี่ , นี้
Is this seat free?
ที่นั่งตรงนี้ว่างไหมครับ

These (det., pron.) เหล่านี้ , พวกนี้  (พหูพจน์ข อง This)
These bags are heavy.
กระเป๋า พวกนี้หนักนะครับ

Those (det., pron.) เหล่านั้น, พวกนั้น (พหูพจน์ข อง That)
How much are those?
ของพวกนั้นราคาเท่าไหร่ครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
About (prep., adv.) เกี่ยวกับ, ประมาณ
What is this book about? หนังสือเล่มนี้เกี่ยวกับอะไรครับ

Above (prep., adv.) เหนือ, ข้างบน
Please look at the picture above.
กรุณาดูรูปภาพด้านบนนะครับ

Across (prep., adv.) ข้าม, ฝั่ งตรงข้าม
The bank is across the street.
ธนาคารอยู่ฝั่ งตรงข้ามถนนครับ

After (prep.) หลังจาก
Let's meet after lunch. เจอกันหลังมื้อเที่ยงนะครับ

Around (prep., adv.) รอบๆ, ประมาณ
Is there an ATM around here?
แถวนี้มีตู้เอทีเอ็มไหมครับ

As (prep.) ในฐานะ, เหมือนกับ
I work as a teacher. ผมทำงานในฐานะครูครับ

At (prep.) ที่, เวลา
I will wait for you at the entrance.
เดี๋ยวผมจะรอคุณอยู่ที่ทางเข้านะครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
Before (prep.) ก่อน
We should get to the airport two hours before
departure.
พวกเราควรไปถึงสนามบินก่อนเวลาเครื่องออกสองชั่วโมงนะ
ครับ (ใช้เตือนเพื่อนร่วมทริปเพื่อกันตกเครื่อง)

Behind (prep., adv.) ข้างหลัง
Is there a garden behind the hotel?
มีสวนอยู่ด้านหลังโรงแรมไหมครับ?

Below (adv., prep.) ข้างล่าง, ต่ำกว่า
Please verify your signature below.
รบกวนตรวจสอบลายเซ็นของคุณด้านล่างด้วยครับ

Between (prep.) ระหว่าง
What is the difference between these two rooms?
ห้องพักสองห้องนี้แตกต่างกันตรงไหนครับ?

Down (adv., prep.) ลง, ไปตาม(ถนน), ลดลง
Go down this street and turn left.
เดินตรงไปตามถนนเส้นนี้แล้วเลี้ยวซ้ายนะครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
During (prep.) ระหว่าง (ช่วงเวลา)
Is it safe to walk around here during the night?
แถวนี้เดินเที่ยวช่วงกลางคืนปลอดภัยไหมครับ?

For (prep.) สำหรับ, เป็นเวลา
I would like to rent a car for three days.
ผมต้องการเช่ารถสักสามวันครับ

From (prep.) จาก
How long does it take to get here from the airport?
เดินทางจากสนามบินมาที่นี่ใช้เวลานานแค่ไหนครับ?

In (prep., adv.) ใน, ในช่วง
Do you have this shirt in a larger size?
คุณมีเสื้อตัวนี้ในไซส์ที่ใหญ่กว่านี้ไหมครับ?

Into (prep.) เข้าไปใน
Please insert your card into the slot.
รบกวนเสียบบัตรเข้าไปในช่องด้วยนะครับ

Like (prep.) เหมือน, เช่น
I am looking for a local restaurant like this one.
ผมกำลังมองหาร้านอาหารท้องถิ่นแบบร้านนี้ครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
Near (prep., adj., adv.) ใกล้
Is there a bank near here? แถวนี้มีธนาคารไหมครับ

Next (adj., adv.) ถัดไป
See you next week. เจอกันสัปดาห์หน้า นะครับ

Next to (prep.) ติดกับ, ถัดจาก
Is there a convenience store next to the hotel?
มีร้านสะดวกซื้ออยู่ติดกับโรงแรมไหมครับ?

Of (prep.) ของ, แห่ง
Could you take a picture of us, please?
รบกวนช่วยถ่ายรูปให้พวกเราหน่อยได้ไหมครับ?

Off (adv., prep.) ออกจาก, ลง(รถ), ปิด
Please drop me off at the corner.
รบกวนช่วยจอดส่งผมตรงหัวมุมด้วยนะครับ

On (prep., adv.) บน, เปิด(เครื่องใช้ไฟฟ้า )
Is the tax included on the menu price?
ภาษีรวมอยู่ในราคาเมนูแล้วหรือยังครับ?



หมวดคำบุพบทและคำระบุตำแหน่ง
Out (adv./prep.) ออก, หมด
I would like to check out now.
ผมต้องการเช็คเอาท์ตอนนี้เลยครับ

Outside (adv.) ข้างนอก
Please wait outside. รบกวนรอข้างนอกสักครู่นะครับ

Over (prep., adv.) เหนือ, ข้าม, เกิน, จบ
We walked over the bridge to get to the market.
พวกเราเดินข้ามสะพานเพื่อไปที่ตลาดครับ

Past (adj., n., prep.) ผ่าน, อดีต
Walk past the cinema. เดินผ่านโรงหนังไปนะครับ

Through (prep., adv.) ผ่าน (ทะลุ), ตลอด
Go through that door. เดินผ่านประตูบานนั้นไปเลยครับ

To (prep.) ถึง, ไปยัง
I am going to the market. ผมกำลังจะไปตลาดครับ

Under (prep., adv.) ใต้, ข้างล่าง, ต่ำกว่า
Please stow your bag under the seat in front of you.
รบกวนเก็บกระเป๋า ไว้ใต้ที่นั่งด้านหน้า ด้วยนะครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
Up (adv., prep.) ขึ้น, ข้างบน
Let's go up. ขึ้นไปข้างบนกันเถอะครับ

With (prep.) กับ, ด้วย
Would you like to come with me?
คุณอยากไปกับผมไหมครับ

Without (prep.) ปราศจาก, โดยไม่มี
Coffee without sugar, please.
ขอกาแฟไม่ใส่น้ำ ตาลนะครับ

Away (adv.) ออกไป, ห่างออกไป
How far away is the city center?
ตัวเมืองอยู่ห่างออกไปไกลแค่ไหนครับ

Downstairs (adv.) ชั้นล่าง
I will meet you downstairs in the lobby.
เดี๋ยวผมลงไปเจอคุณชั้นล่างตรงล็อบบี้นะครับ

Far (adv.) ไกล
Is it far from here? มันไกลจากที่นี่ไหมครับ



หมวดคำบุพบทและคำระบุตำแหน่ง
Here (adv.) ที่นี่ , ตรงนี้
Can I leave my luggage here for a few hours?
ผมขอฝากกระเป๋า เดินทางไว้ตรงนี้สักสองสามชั่วโมงได้ไหม
ครับ

Inside (prep., adv.) ข้างใน
It is cool inside. ข้างในอากาศเย็นสบายครับ

There (adv.) ที่นั่น, ตรงนั้น
Can we sit over there by the window?
พวกเราขอนั่งตรงโน้น  ริมหน้า ต่างได้ไหมครับ

Upstairs (adv.) ชั้นบน
Is the swimming pool upstairs?
สระว่ายน้ำ อยู่ชั้นบนใช่ไหมครับ



หมวดคำเชื่อมและคำกริยาวิเศษณ์ข ยายความ

So (adv., conj.) ดังนั้น, มาก
It was raining, so I took a taxi.
ฝนตก ผมเลยนั่งแท็กซี่มาครับ

Than (conj.) กว่า (เปรียบเทียบ)
This one is better than that one. อันนี้ดีกว่าอันนั้นครับ

Until (conj./prep.) จนกระทั่ง, จนถึง
Can I leave my luggage here until check-in time?
ผมขอฝากกระเป๋า เดินทางไว้ที่นี่จนกว่าจะถึงเวลาเช็คอินได้
ไหมครับ

While (conj.) ในขณะที่, ระหว่างที่
Could you watch my bag while I go to the restroom?
รบกวนช่วยดูกระเป๋า ให้ผมหน่อยระหว่างที่ผมไปเข้าห้องน้ำ
ได้ไหมครับ

However (adv.) อย่างไรก็ตาม
It is raining; however, we must go.
ฝนตก แต่อย่างไรก็ตามเราต้องไปครับ

Also (adv.) ด้วย, เช่นกัน, อีกด้วย
I speak Thai, and also English.
ผมพูดภาษาไทย และภาษาอังกฤษด้วยครับ



หมวดคำเชื่อมและคำกริยาวิเศษณ์ข ยายความ

Even (adv.) แม้แต่, แม้กระทั่ง
This shop is open even on Sundays.
ร้านนี้เปิดแม้กระทั่งวันอาทิตย์เลยครับ

Just (adv.) เพิ่งจะ, แค่
I am just looking, thank you.
ผมแค่ขอเดินดูเฉยๆ ขอบคุณครับ

Maybe (adv.) อาจจะ, บางที
Maybe we can come back later.
บางทีเราค่อยกลับมาใหม่อีกทีก็ได้ครับ

Only (adj., adv.) เท่านั้น, เพียงแค่
Do you accept credit cards, or is it cash only?
รับบัตรเครดิตไหมครับ หรือว่ารับเฉพาะเงินสด

Perhaps (adv.) บางที (สุภาพกว่า Maybe)
Perhaps there is a misunderstanding.
บางทีอาจจะมีการเข้าใจผิดกันก็ได้ครับ



หมวดคำเชื่อมและคำกริยาวิเศษณ์ข ยายความ

Probably (adv.) น่าจะ, คงจะ (มีความเป็นไปได้สูงกว่า
Maybe)
We will probably arrive a bit late.
พวกเราน่าจะไปถึงช้าหน่อยนะครับ

Quite (adv.) ค่อนข้าง
Is it quite far from here?
มันอยู่ค่อนข้างไกลจากที่นี่ไหมครับ? (ใช้ถามทางเพื่อตัดสิน
ใจว่าควรเดินไปหรือเรียกแท็กซี่ดี)

Really (adv.) จริงๆ
I really appreciate your help.
ผมซาบซึ้งในความช่วยเหลือของคุณจริงๆ ครับ

Well (adv., adj., exclam.) ดี, สบายดี, ก็... (คำอุทาน)
I hope you are well. หวังว่าคุณคงสบายดีนะครับ



หมวดกริยาช่วยและ Modal Verbs ที่ใช้บ่อย

Be (v., auxiliary v.) เป็น, อยู่, คือ
I will be right back. เดี๋ยวผมจะรีบกลับมานะครับ

Could (modal v.) สามารถ, ช่วย...ได้ไหม (สุภาพกว่า Can)
Could you help me, please?
รบกวนช่วยผมหน่อยได้ไหมครับ

Have to (modal v.) จำเป็นต้อง
I have to go now.
ผมต้องขอตัวก่อนนะครับ (จำเป็นต้องไปแล้ว)

Shall (modal v.) จะ, ...กันไหม (ใช้เสนอแนะ)
Shall we take a taxi or walk?
เราจะนั่งแท็กซี่หรือเดินไปกันดีครับ?

Would (modal v.) จะ, อยากจะ, รบกวน...ไหม (ใช้ขอร้อง)
Would you mind taking a photo for us?
รบกวนช่วยถ่ายรูปให้พวกเราหน่อยได้ไหมครับ?



หมวดการสื่อสารและข้อมูล
Article (n.) บทความ
I read an article recommending this restaurant.
ผมอ่านเจอ(บทความ)รีวิวแนะนำร้านอาหารร้านนี้ครับ

Interview (n., v.) การสัมภาษณ์,  สัมภาษณ์
I have to go to the embassy for a visa interview.
ผมต้องไปสถานทูตเพื่อสัมภาษณ์วี ซ่าครับ

Opinion (n.) ความคิดเห็น
In your opinion, which island is the most beautiful?
ในความคิดเห็นของคุณ เกาะไหนสวยที่สุดครับ

Paragraph (n.) ย่อหน้า
Please read this paragraph about the insurance policy.
รบกวนอ่านย่อหน้า นี้เกี่ยวกับเงื่อนไขประกันภัยหน่อยนะครับ 

Statement (n.) คำแถลง, รายการเดินบัญชี
I need to print my bank statement for the visa
application.
ผมต้องปรินท์รายการเดินบัญชีเพื่อใช้ยื่นขอวีซ่าครับ

Topic (n.) หัวข้อ
Politics is a sensitive topic in this country.
การเมืองเป็นหัวข้อที่ละเอียดอ่อนในประเทศนี้นะครับ



หมวดนันทนาการ
Culture (n.) วัฒนธรรม
I love Thai culture. ผมชอบวัฒนธรรมไทยครับ

Festival (n.) เทศกาล
Are you going to the festival?
คุณจะไปงานเทศกาลไหมครับ

Programme (n.) รายการ (ทีวี/วิทยุ), แผนงาน
What is your favourite programme?
รายการโปรดของคุณคือรายการอะไรครับ

Vacation (n.) วันหยุดยาว, การลาพักร้อน
I need a vacation. ผมต้องการลาพักร้อนครับ



หมวดลำดับเวลา (Timeline)
Beginning (n.) การเริ่มต้น
This is just the beginning. นี่เป็นเพียงการเริ่มต้นครับ

Century (n.) ศตวรรษ (100 ปี)
This castle is from the 15th century.
ปราสาทนี้สร้างมาตั้งแต่ศตวรรษที่ 15 ครับ

Geography (n.) ภูมิศาสตร์
I am studying geography.
ผมกำลังศึกษาเรื่องภูมิศาสตร์ครับ

Journey (n.) การเดินทาง (ระยะยาว)
Have a safe journey. ขอให้เดินทางโดยสวัสดิภาพนะครับ

Period (n.) ช่วงเวลา, คาบเรียน
It was a difficult period.
มันเป็นช่วงเวลาที่ยากลำบากครับ



หมวดลำดับเวลา (Timeline)
Routine (n.) กิจวัตร, ขั้นตอนปกติ
Do not worry, it is just a routine security check.
ไม่ต้องกังวลครับ มันเป็นแค่การตรวจความปลอดภัยตาม
ปกติ (ใช้บอกเพื่อนร่วมทริปเวลาเจอเจ้าหน้า ที่เรียกตรวจที่
สนามบิน)

View (n.) วิว, ทิวทัศน์,  มุมมอง
The view from here is great. วิวจากตรงนี้สวยมากครับ



หมวดเศรษฐกิจ
Capital (n., adj.) เมืองหลวง, เงินทุน
What is the capital of Japan?
เมืองหลวงของญี่ปุ่น คือที่ไหนครับ

Chart (n.) แผนภูมิ
Do you have a size chart for these shirts?
คุณมีตารางเทียบไซส์สำหรับเสื้อเชิ้ตพวกนี้ไหมครับ
(ใช้ถามพนักงานเวลาซื้อเสื้อผ้าในต่างประเทศแล้วไม่แน่ใจ
ขนาด)

Model (n.) แบบจำลอง, นางแบบ/นายแบบ, รุ่น
Does this model come with an international
warranty?
สินค้ารุ่นนี้มีการรับประกันแบบทั่วโลกไหมครับ
(ประโยคนี้สำคัญมากสำหรับนักท่องเที่ยว เวลาซื้อนาฬิกา
กล้อง หรือเครื่องใช้ไฟฟ้า  จะได้เคลมศูนย์ไทยได้)

Success (n.) ความสำเร็จ
I wish you success. ขอให้คุณประสบความสำเร็จนะครับ



หมวดความรู้ทั่วไป
Diet (n.) อาหารการกิน, การคุมอาหาร
I am on a diet. ผมกำลังคุมอาหารอยู่ครับ

Difference (n.) ความแตกต่าง
What is the difference? ความแตกต่างคืออะไรครับ

Example (n.) ตัวอย่าง
For example, apples and oranges.
ตัวอย่างเช่น แอปเปิลและส้มครับ

Interest (n., v.) ความสนใจ, ดอกเบี้ย
I have an interest in game.
ผมมีความสนใจในเกมครับ

Kind (n.) ประเภท, ชนิด (ถ้า adj. แปลว่า ใจดี)
What kind of food do you like?
คุณชอบอาหารประเภทไหนครับ

Lot (pron., det., adv.) มาก, จำนวนมาก
Thanks a lot. ขอบคุณมากครับ



หมวดความรู้ทั่วไป
Mistake (n.) ข้อผิดพลาด
Everyone makes mistakes.
ใครๆ ก็ทำผิดพลาดกันได้ครับ

Order (n., v.) คำสั่ง, สั่งอาหาร
I would like to order room service.
ผมต้องการสั่งรูมเซอร์วิสครับ
(ใช้โทรสั่งอาหารขึ้นมาทานบนห้องพัก)

Pair (n.) คู่
I need a new pair of shoes.
ผมต้องการรองเท้าคู่ใหม่ครับ

Part (n.) ส่วน, ย่าน
Which part of the city is best for nightlife?
ส่วนไหน (ย่านไหน) ของเมืองที่เหมาะกับไนท์ไลฟ์ที่สุดครับ

Piece (n.) ชิ้น, ใบ (ลักษณนามกระเป๋า )
Can I have a piece of pizza?
ขอพิซซ่าชิ้นหนึ่งได้ไหม

Plan (n., v.) แผนการ (n.), วางแผน (v.)
Everything is going according to plan.
ทุกอย่างเป็นไปตามแผนครับ



หมวดความรู้ทั่วไป
Point (n.) จุด, ประเด็น
Where is the meeting point?
จุดนัดพบอยู่ที่ไหนครับ

Practice (n.) การฝึกฝน
Practice makes perfect.
การฝึกฝนทำให้เก่งขึ้นครับ

Reason (n.) เหตุผล
What is the reason for the delay?
สาเหตุของความล่าช้าคืออะไรครับ

Section (n.) ส่วน, แผนก
I would like a table in the non-smoking section.
ผมขอโต๊ะในโซนไม่สูบบุหรี่นะครับ

Situation (n.) สถานการณ์
It is a difficult situation.
เป็นสถานการณ์ที่ ลำบากนะครับ

Way (n.) ทาง, วิธี
Could you show me the way on the map?
รบกวนช่วยชี้ทางในแผนที่ให้ผมหน่อยได้ไหมครับ`;

// ตัวแปรสำหรับเก็บข้อมูลที่ Parse แล้ว
let flashcards = [];
let categories = new Set();
let currentCategoryCards = [];
let currentIndex = 0;
let isFlipped = false;
let knownCards = new Set();

// ฟังก์ชัน Parse ข้อมูลดิบ
function parseData(text) {
    const lines = text.split('\n');
    let currentCategory = "";
    let currentCard = null;
    let buffer = [];

    lines.forEach((line) => {
        line = line.trim();
        if (!line) return;

        if (line.startsWith("หมวด")) {
            currentCategory = line;
            categories.add(currentCategory);
        } else {
            // ตรวจสอบว่าเป็นบรรทัดคำศัพท์หรือไม่ (มีภาษาอังกฤษ และวงเล็บประเภทคำ)
            // Regex: เริ่มต้นด้วยตัวอักษร, มีวงเล็บเปิดปิดที่มีจุดข้างใน
            // เช่น Aunt (n.)
            const isVocabLine = /^[A-Z][a-zA-Z\s/-]+\s*\(.+\)/.test(line);

            if (isVocabLine) {
                // บันทึกการ์ดเก่าถ้ามี
                if (currentCard) {
                    flashcards.push(currentCard);
                }

                // เริ่มการ์ดใหม่
                // แยกคำศัพท์และคำแปล
                // ตัวอย่าง: Aunt (n.) ป้า , น้า , อา
                // หาตำแหน่งวงเล็บปิดสุดท้าย
                const lastParenIndex = line.lastIndexOf(')');
                const vocabPart = line.substring(0, lastParenIndex + 1).trim();
                const meaningPart = line.substring(lastParenIndex + 1).trim();

                // แยก Vocab และ Type จาก vocabPart
                // Aunt (n.) -> Vocab: Aunt, Type: (n.)
                const firstParenIndex = vocabPart.indexOf('(');
                const vocab = vocabPart.substring(0, firstParenIndex).trim();
                const type = vocabPart.substring(firstParenIndex).trim();

                currentCard = {
                    id: flashcards.length,
                    category: currentCategory,
                    vocab: vocab,
                    type: type,
                    meaning: meaningPart,
                    exampleEn: "",
                    exampleTh: ""
                };
                buffer = []; // Reset buffer for examples
            } else if (currentCard) {
                // ถ้าไม่ใช่บรรทัดศัพท์ จะเป็นประโยคตัวอย่าง
                // บรรทัดแรกหลังศัพท์ คือ En, ที่เหลือคือ Th
                if (!currentCard.exampleEn) {
                    currentCard.exampleEn = line;
                } else {
                    // สะสมประโยคไทย (บางทีมีหลายบรรทัด)
                    if (currentCard.exampleTh) {
                        currentCard.exampleTh += " " + line;
                    } else {
                        currentCard.exampleTh = line;
                    }
                }
            }
        }
    });

    // ดันการ์ดใบสุดท้าย
    if (currentCard) {
        flashcards.push(currentCard);
    }
}

// เริ่มต้นระบบ
function initApp() {
    parseData(rawDataContent);

    // Populate Category Dropdown
    const select = document.getElementById('category-select');
    categories.forEach(cat => {
        const option = document.createElement('option');
        option.value = cat;
        option.textContent = cat;
        select.appendChild(option);
    });

    // Event Listeners
    select.addEventListener('change', (e) => {
        filterCards(e.target.value);
    });

    document.addEventListener('keydown', (e) => {
        if (e.key === "ArrowRight") nextCard();
        if (e.key === "ArrowLeft") prevCard();
        if (e.key === " " || e.key === "Enter") flipCard();
    });

    // Start with all cards
    filterCards('all');
}

// กรองการ์ดตามหมวดหมู่
function filterCards(category) {
    if (category === 'all') {
        currentCategoryCards = flashcards.filter(c => !knownCards.has(c.id));
    } else {
        currentCategoryCards = flashcards.filter(c => c.category === category && !knownCards.has(c.id));
    }

    document.getElementById('total-count').innerText = currentCategoryCards.length;
    currentIndex = 0;
    resetCardState();
    updateCardDisplay();
}

// สุ่มการ์ด
function shuffleCards() {
    for (let i = currentCategoryCards.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [currentCategoryCards[i], currentCategoryCards[j]] = [currentCategoryCards[j], currentCategoryCards[i]];
    }
    currentIndex = 0;
    resetCardState();
    updateCardDisplay();
}

// อัปเดตหน้าจอแสดงผล
function updateCardDisplay() {
    const container = document.getElementById('flashcard');
    const btnNext = document.getElementById('btn-next');
    const btnPrev = document.getElementById('btn-prev');

    if (currentCategoryCards.length === 0) {
        // กรณีไม่มีการ์ดเหลือเลย
        document.getElementById('card-vocab').innerText = "เก่งมาก!";
        document.getElementById('card-category').innerText = "Completed";
        document.getElementById('card-vocab-back').innerText = "จำได้หมดแล้ว";
        document.getElementById('card-meaning').innerText = "คุณจำศัพท์ในหมวดนี้ได้หมดแล้ว";
        document.getElementById('card-ex-en').innerText = "";
        document.getElementById('card-ex-th').innerText = "กดปุ่ม 'รีเซ็ต' เพื่อเริ่มใหม่";
        btnNext.disabled = true;
        btnPrev.disabled = true;
        return;
    }

    const card = currentCategoryCards[currentIndex];

    // Front
    document.getElementById('card-category').innerText = card.category;
    // Use Example EN as the main question. If empty, fallback to vocab (though most have examples).
    // Logic: Show Sentence
    document.getElementById('card-vocab').innerText = card.exampleEn ? `"${card.exampleEn}"` : card.vocab;

    // Back
    // Show Vocab + Meaning + Thai Sentence
    document.getElementById('card-vocab-back').innerText = `${card.vocab} ${card.type}`;
    document.getElementById('card-meaning').innerText = card.meaning;
    // Show Thai sentence
    document.getElementById('card-ex-th').innerText = card.exampleTh || "-";
    // Show English sentence small for reference
    document.getElementById('card-ex-en').innerText = card.exampleEn ? `"${card.exampleEn}"` : "-";

    // Update Progress
    document.getElementById('progress-text').innerText = `${currentIndex + 1} / ${currentCategoryCards.length}`;
    const progressPercent = ((currentIndex + 1) / currentCategoryCards.length) * 100;
    document.getElementById('progress-bar').style.width = `${progressPercent}%`;

    // Button states
    btnPrev.disabled = currentIndex === 0;
    btnNext.disabled = currentIndex === currentCategoryCards.length - 1;
}

// พลิกการ์ด
function flipCard() {
    if (currentCategoryCards.length === 0) return;
    const card = document.getElementById('flashcard');
    isFlipped = !isFlipped;
    if (isFlipped) {
        card.classList.add('flipped');
    } else {
        card.classList.remove('flipped');
    }
}

function resetCardState() {
    isFlipped = false;
    document.getElementById('flashcard').classList.remove('flipped');
}

function nextCard() {
    if (currentIndex < currentCategoryCards.length - 1) {
        currentIndex++;
        resetCardState();
        setTimeout(updateCardDisplay, 200); // รอ animation นิดนึงถ้ามีการเปลี่ยน
    }
}

function prevCard() {
    if (currentIndex > 0) {
        currentIndex--;
        resetCardState();
        setTimeout(updateCardDisplay, 200);
    }
}

// ฟังก์ชัน "จำได้แล้ว"
function markAsKnown() {
    if (currentCategoryCards.length === 0) return;

    const cardId = currentCategoryCards[currentIndex].id;
    knownCards.add(cardId);

    // เอามันออกจากลิสต์ปัจจุบัน
    currentCategoryCards.splice(currentIndex, 1);

    // ปรับ index
    if (currentIndex >= currentCategoryCards.length) {
        currentIndex = Math.max(0, currentCategoryCards.length - 1);
    }

    document.getElementById('total-count').innerText = currentCategoryCards.length;
    resetCardState();
    updateCardDisplay();
}

// รีเซ็ตความจำ
function resetProgress() {
    if (confirm("คุณต้องการรีเซ็ตคำที่จำได้แล้วทั้งหมดหรือไม่?")) {
        knownCards.clear();
        const select = document.getElementById('category-select');
        filterCards(select.value);
        alert("รีเซ็ตเรียบร้อยแล้ว!");
    }
}

// เริ่มต้น
initApp();